...
app.setName('cloud-n-cream-pos');

// We need to run this within Electron to have access to app.getPath('userData')
app.whenReady().then(async () => {
    try {
        const { createIsolatedSupabaseClient } = require('./dist/electron/supabase');
        const { getCachedTenant, getUnsyncedOperations, markOperationSynced, markOperationError } = require('./dist/electron/sqliteDatabase');
